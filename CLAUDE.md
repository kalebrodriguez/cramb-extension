...
- 2026-06-23 — M5 a11y axe gate. Stood up Playwright e2e (`tests/e2e/`, `playwright.config.ts`) that loads the built MV3 extension (`channel: 'chromium'`) and runs `@axe-core/playwright` over popup/sidepanel/options/onboarding in **both** themes (WCAG 2.1 A/AA). Fixed every violation it found: `--text-faint` failed AA contrast in both themes (dark `#64748b`→`#8b97ac`; light `#94a3b8`→`#5c6b81`), and Options form controls lacked associated `<label htmlFor>`. Wired the gate into CI (build → `playwright install --with-deps chromium` → `pnpm test:e2e`). New dev deps pinned: `@playwright/test@1.55.1` (patched), `@axe-core/playwright@4.10.1`. 8/8 a11y cases green.
- 2026-06-23 — M5 theme. Light theme now actually applies: `lib/theme.ts` (`initTheme`/`applyTheme`/`resolveTheme`) reads `appearance.theme` (dark/light/system), applied on every UI entrypoint's `main.tsx`, follows `prefers-color-scheme` while 'system', and live-updates via `chrome.storage.onChanged` (exported `SETTINGS_KEY`). Options gains an Appearance theme switcher (live-applies + persists immediately). Tokenized all remaining raw palette colors (rating bar → `text-rating-*`, destructive → `text-danger`) so light renders correctly. Confirmed the side panel already wraps at 320px (`flex-wrap`/`min-w-0`). Full CI sequence (typecheck/lint/test/build/build:firefox) green; 69 tests.
- 2026-06-23 — M5 toasts. Added a tiny Zustand toast store (`lib/toast.ts`) + `components/ToastViewport.tsx` (variant-styled, auto-dismiss, `role=alert/status`, reduced-motion via `.toast-enter` CSS). Mounted in the side panel and popup; replaced all 8 native `alert()`s with `toast.error()`/`toast.success()` (kept the 3 destructive `window.confirm()` guards and DataSection's inline status). 69 tests pass (added `tests/lib/toast.test.ts`); typecheck/lint/build clean.
- 2026-06-23 — M5 onboarding wizard. New `src/entrypoints/onboarding/` (4 steps: welcome → connect+`model.test` → live first-card demo → capture tips), opened on first install via `chrome.runtime.onInstalled` (`reason === 'install'`). Added `generate.fromText` message + handler (demo generates from a built-in passage; nothing persisted, no auto-save — golden rule §5). Extracted `lib/providers-meta.ts` (PROVIDERS/DEFAULT_MODELS/KEY_HELP) shared by Options + onboarding so model ids can't drift. typecheck/lint/build clean; 65 tests pass.
